...
namespace MovieMakerV1
{
    class TicketManger
    {
        ...
        //stores the snack and drinks ordered by the most recent ticket holder
        public void AddSnacksDrinkOrder(List<int> sOrder, List<int> sQuantity, List<int> dOrder, List<int> dQuantity)
        {
            //if index =list.Count - It then would refer to the last element in the list
            ticketHolders[ticketHolders.Count - 1].AddSnack(sOrder, sQuantity);
            ticketHolders[ticketHolders.Count - 1].AddDrink(dOrder, dQuantity);
        }
